...
- `{project}/docs/requirements.md` — ターゲット・CTA・構成仕様
- `{project}/docs/DESIGN.md` — デザイン方向性ドラフト（v1）。カラー・世界観・参考サイト・レイアウト好みの根拠
...
`DESIGN.md`（v1）に記載された以下の方向性をビジュアルに反映する:

- 雰囲気・世界観 → 全体のトーン設計
- カラー方向性 → カラーパレット選定
- 参考サイト → スタイルの参考元
- レイアウト好み → 余白・画像比率・密度の調整
- Do's and Don'ts → デザイン判断の基準
...
| 8 | **ファイルは必ず上書き（write）する。追記（append）禁止。** 修正・再生成時も、完全なHTML文書1つだけで `design-draft.html` を上書きすること。`<!DOCTYPE html>` が2回以上出現するファイルは壊れたファイルである |
...
Phase 3 完了時、`DESIGN.md` を方向性ドラフト（v1）から確定版（v2）に更新する。
実際に使ったデザイントークンを具体値で記録する。

### 更新手順

1. `design-draft.html` 冒頭の DESIGN TOKENS コメントから値を抽出する
2. `DESIGN.md` の各セクションを具体値で上書きする
3. ステータスを「確定版（v2）」に変更する

### v2 で追加・具体化する項目

```markdown
ステータス: 確定版（v2） ← ステータスを変更

## 2. カラーパレット（確定値）

| 用途 | カラー | 根拠 |
|------|--------|------|
| Primary | #XXXXXX | （なぜこの色か） |
| Secondary | #XXXXXX | |
| Background | #XXXXXX | |
| Text | #XXXXXX | |
| Accent/CTA | #XXXXXX | |
...
- **見出しフォント:** [フォント名]（根拠: ○○）
- **本文フォント:** [フォント名]
- **スケール:** Hero=XXpx, H2=XXpx, H3=XXpx, Body=XXpx
